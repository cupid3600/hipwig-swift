...
internal class PermissionsView: UIView {
    ...
    func configureInView(_ view: UIView, title: String, description: String, completion: @escaping ButtonAction) {
        ...
        topView.frame = CGRect(x: 0, y: 0, width: view.bounds.width, height: 66)
        ...
        closeButton.setImage(UIImage(named: "libraryCancel", in: CameraGlobals.shared.bundle, compatibleWith: nil), for: UIControl.State())
        closeButton.sizeToFit()

        let size = topView.frame.size
        let closeSize = closeButton.frame.size
        
        closeButton.frame.origin = CGPoint(x: size.width - closeSize.width - 20, y: closeSize.height / 2 + verticalSpacing)
    }
