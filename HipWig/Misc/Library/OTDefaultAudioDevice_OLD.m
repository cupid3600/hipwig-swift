...
#import "OTDefaultAudioDevice_OLD.h"
...
//    SetVoiceAUQuality(AudioUnit inVoiceUnit, <#UInt32 inQuality#>)
...
void SetVoiceAUQuality(AudioUnit inVoiceUnit, UInt32 inQuality)
{
    if ((inVoiceUnit == NULL) || (inQuality > 127))
        return ;//paramErr;
...
static void update_recording_delay(OTDefaultAudioDevice_OLD* device) {
                             ...
                             AudioBufferList *data)
{
    ...
    OTDefaultAudioDevice_OLD *dev = (__bridge OTDefaultAudioDevice_OLD*) ref_con;
...
static void update_playout_delay(OTDefaultAudioDevice_OLD* device) {
...
- (BOOL)isHeadsetPluggedIn {
    ...
    for (AVAudioSessionPortDescription* desc in [route outputs]) {
        if ([[desc portType] isEqualToString:AVAudioSessionPortHeadphones] || [[desc portType] isEqualToString:AVAudioSessionPortBluetoothHFP] || [[desc portType] isEqualToString:AVAudioSessionPortBluetoothA2DP])
            return YES;
    }
    return NO;
}
...
- (BOOL)setupAudioUnit:(AudioUnit *)voice_unit playout:(BOOL)isPlayout;
{
    ...
    SetVoiceAUQuality(*voice_unit, 127);
