...
open class ProgressBar: UIView {
    ...
    @IBInspectable public var upperBound: CGFloat = 0
    
    @IBInspectable public var progressValue: CGFloat = 0 {
        didSet {
            progressValue = progressValue.clamped(lowerBound: 0, upperBound: upperBound)
            setNeedsDisplay()
        }
    }
    ...
    func calculatePercentage() -> CGFloat {
        ...
        let progress = ((progressValue / upperBound) * screenWidth)
